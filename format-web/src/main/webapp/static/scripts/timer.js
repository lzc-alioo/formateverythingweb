...
function drawClock(startTime, endTime, formatObj) {
    ctx.clearRect(0, 0, 500, 500);  //清空整个画布
    ...
    drawNumber(); //刻度上面的数字
    drawScale(300, 2, "#4c4c4c", 1.2, 0, 172, 0, 180);      //画1/5秒刻度，每秒再分5档
    drawScale(60, 2, "#ffffff", 6, 0, 164, 0, 180);         //画秒刻度

    drawPointer(2, "red", 1.2, milliSecFix, 0, 20, 0, -180);       //画秒针
...
function showHistoryData() {
    ...
    $.each(formatObjArr,function (i, obj) {
        ...
        var str="<div><span class='history-tip'>计次"+(i+1)+"</span><span class='history-text'>" + text + "</span></div>"
...
//    }
//});
$(".history-data").on("dblclick","div",function(){
    var historyDataOne=$(this);
    if(historyDataOne.find(".tip").length>=0){
        //
        // var idx=parseInt(historyDataOne.parent().find('.history-tip').text())-1;
        var idx=parseInt(historyDataOne.find('.history-tip').text().substring(2))-1;
...
function doRun(){
    if (jishiqiTimer != null) {
        ...
        $(".runner div").html("开始");
        $(".runner").removeClass("runner-stop").addClass("runner-start");
    } else {
        startTime = new Date().getTime();
        jishiqiTimer = setInterval(mystart, 123);

        $(".runner div").html("停止");
        $(".runner").removeClass("runner-start").addClass("runner-stop");
    }
}
