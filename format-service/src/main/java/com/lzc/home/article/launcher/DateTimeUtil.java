...
public class DateTimeUtil {
     ...
     * @return String 格式化如 yyyy-MM-dd
     */
    public static Date getNextDateTimeBySecond(int seconds) {
        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.SECOND, seconds);
    ...
    public static int[] getNextDay(int year, int month, int day, int days) {
        ...
        // * days);
        long longDate = (calendar.getTime()).getTime() + (1000L * 60 * 60 * 24 * days);
        Date date = new Date(longDate);
        calendar.setTime(date);
    ...
    /**
     * 字符串转为时间,字符串符合标准格式:"YYYY-MM-DD HH:MM:SS" </br>
     *
     * @param dateTime 标准时间格式 "YYYY-MM-DD HH:MM:SS"
     * @return java.util.Date
     * @History: 2015年8月27日 下午5:59:41 柳志崇 <br>
     * 毫秒控制的不精确，不推荐大家使用，建议大家改用toDateFromStr方法
     */
    @Deprecated
